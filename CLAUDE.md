...
| `fullstack-image/` | `ghcr.io/{owner}/fullstack` | 全栈开发环境 + Android SDK (运行时安装) + Rust/Tauri 编译环境 (固定版本), Git via PPA, 内置安全扫描 (gitleaks + betterleaks) 与 pre-commit, GUI 桌面运行时 (WSLg 直通: 中文字体/fcitx5 输入法/ZCode) |
